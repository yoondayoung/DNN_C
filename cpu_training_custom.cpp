...
layer_info parsing_cmd_args(int num, std::string model_info){
    ...
    switch(num){
        case Conv2d:
            ...
            //in_h, in_w, out_h, out_w 에 대한 처리는 인풋에 따라 달라서 어떻게 계산할 지 정해주셈
...
prv_layer create_bwd_fc_layer(dnnl_engine_t engine, uint32_t *n_bwd, dnnl_primitive_t *net_bwd, args_t *net_bwd_args, layer_info layer_i, float *net_diff_dst){
        // std::cout << "create bwd fc layer: in c" << layer_i.in_c << ",in h " << layer_i.in_h << ",in w " << layer_i.in_w <<  ",out c " << layer_i.out_c << ", out h" << layer_i.out_h << ", out w" << layer_i.out_w << std::endl;

        // 이게 필요할지는 봐야될듯 아마 마지막에 쓰긴할텐데 -> reorder도 해야됨
        dnnl_memory_t fc_user_diff_dst_memory;
        if(layer_i.layer_order==Last){
                // create memory for user diff dst data
                dnnl_dims_t last_dst_sizes = {BATCH, LIN_OC};
                init_data_memory(2, last_dst_sizes, dnnl_nc, engine, net_diff_dst,
                        &fc_user_diff_dst_memory);
        }

        // Backward fully-connected with respect to weights
        ...
        // dst dim {N, OC}
        dnnl_dims_t fc_dst_sizes = {BATCH, layer_i.out_c};
        float *fc_diff_bias_buffer = (float *)mlm_malloc(product(fc_bias_sizes, 1) * sizeof(float),1);
        float *fc_user_diff_weights_buffer = (float *)mlm_malloc(product(fc_weights_sizes, 4) * sizeof(float),1);

        // initialize memory for diff weights in user format
        dnnl_memory_t fc_user_diff_weights_memory;
        init_data_memory(4, fc_weights_sizes, dnnl_oihw, engine, fc_user_diff_weights_buffer, &fc_user_diff_weights_memory);

        // create bwd fc primitive descriptor
        dnnl_primitive_desc_t fc_bwd_weights_pd;
        {
                // memory descriptors should be in format 'any' to allow backward fc for weights to chose the format it prefers for best performance -> ??
                dnnl_memory_desc_t fc_diff_src_md, fc_diff_weights_md, fc_diff_bias_md, fc_diff_dst_md;
                CHECK(dnnl_memory_desc_reshape(&fc_diff_src_md, layer_i.clayer.src_md, ndims, fc_src_sizes));
                CHECK(dnnl_memory_desc_create_with_tag(&fc_diff_weights_md, 4, fc_weights_sizes, dnnl_f32, dnnl_format_tag_any));
                CHECK(dnnl_memory_desc_create_with_tag(&fc_diff_bias_md, 1, fc_bias_sizes, dnnl_f32, dnnl_format_tag_any));
                CHECK(dnnl_memory_desc_create_with_tag(&fc_diff_dst_md, 2, fc_dst_sizes, dnnl_f32, dnnl_format_tag_any));

                // create bwd fc descriptor
                CHECK(dnnl_inner_product_backward_weights_primitive_desc_create(
                        &fc_bwd_weights_pd, engine, fc_diff_src_md,
                        fc_diff_weights_md, fc_diff_bias_md, fc_diff_dst_md,
                        layer_i.clayer.fwd_hint, NULL));
                CHECK(dnnl_memory_desc_destroy(fc_diff_src_md));
                CHECK(dnnl_memory_desc_destroy(fc_diff_weights_md));
                CHECK(dnnl_memory_desc_destroy(fc_diff_bias_md));
                CHECK(dnnl_memory_desc_destroy(fc_diff_dst_md));
        }

        // dnnl_primitive_t fc_bwd_reorder_src;
        // dnnl_memory_t fc_bwd_internal_src_memory;
        // const_dnnl_memory_desc_t fc_diff_src_md = dnnl_primitive_desc_query_md(
        //     fc_bwd_weights_pd, dnnl_query_src_md, 0);
        // dnnl_memory_desc_t tmp_fc_diff_src_md;
        // CHECK(dnnl_memory_desc_reshape(&tmp_fc_diff_src_md, layer_i.clayer.src_md, ndims, fc_src_sizes));
        // dnnl_memory_desc_destroy(tmp_fc_diff_src_md);
        // const_dnnl_memory_desc_t fc_diff_src_md;
        // dnnl_memory_get_memory_desc(layer_i.clayer.src_memory, &fc_diff_src_md);
        // std::cout << "before reorder" << std::endl;
        // CHECK(prepare_reorder(&layer_i.clayer.src_memory, fc_diff_src_md, engine, 1,
        //         &fc_bwd_internal_src_memory, &fc_bwd_reorder_src, n_bwd,
        //         net_bwd, net_bwd_args));
        // std::cout << "after reorder" << std::endl;
        // dnnl_memory_t fc_bwd_weights_src_memory = fc_bwd_internal_src_memory
        //         ? fc_bwd_internal_src_memory
        //         : layer_i.clayer.src_memory;
        // dnnl_memory_t fc_bwd_weights_src_memory = layer_i.clayer.src_memory;

        dnnl_primitive_t fc_reorder_diff_dst;
        dnnl_memory_t fc_internal_diff_dst_memory;
        const_dnnl_memory_desc_t fc_diff_dst_md = dnnl_primitive_desc_query_md(
                fc_bwd_weights_pd, dnnl_query_diff_dst_md, 0);
        dnnl_memory_t fc_diff_dst_memory;
        if(layer_i.layer_order==Last){
                CHECK(prepare_reorder(&fc_user_diff_dst_memory, fc_diff_dst_md, engine, 1,
                        &fc_internal_diff_dst_memory, &fc_reorder_diff_dst, n_bwd,
                        net_bwd, net_bwd_args));
                fc_diff_dst_memory = fc_internal_diff_dst_memory
                        ? fc_internal_diff_dst_memory
                        : fc_user_diff_dst_memory;
        }
        else{
                CHECK(prepare_reorder(&layer_i.player.src_memory, fc_diff_dst_md, engine, 1,
                        &fc_internal_diff_dst_memory, &fc_reorder_diff_dst, n_bwd,
                        net_bwd, net_bwd_args));
                fc_diff_dst_memory = fc_internal_diff_dst_memory
                        ? fc_internal_diff_dst_memory
                        : layer_i.player.src_memory;
        }

        // create reorder primitives for fc diff weights memory
        dnnl_primitive_t fc_reorder_diff_weights;
        dnnl_memory_t fc_internal_diff_weights_memory;
        const_dnnl_memory_desc_t fc_diff_weights_md = dnnl_primitive_desc_query_md(
                fc_bwd_weights_pd, dnnl_query_diff_weights_md, 0);
        
        if(layer_i.layer_order==Last){
                *n_bwd += 1;
        }

        CHECK(prepare_reorder(&fc_user_diff_weights_memory, fc_diff_weights_md,
            engine, 0, &fc_internal_diff_weights_memory,
            &fc_reorder_diff_weights, n_bwd, net_bwd, net_bwd_args));
        
        if(layer_i.layer_order==Last){
                *n_bwd -= fc_reorder_diff_weights ? 2 : 1;
        }

        dnnl_memory_t fc_diff_weights_memory = fc_internal_diff_weights_memory
            ? fc_internal_diff_weights_memory
            : fc_user_diff_weights_memory;
        // create memory for diff bias memory
        dnnl_memory_t fc_diff_bias_memory;
        const_dnnl_memory_desc_t fc_diff_bias_md = dnnl_primitive_desc_query_md(
                fc_bwd_weights_pd, dnnl_query_diff_weights_md, 1);
        CHECK(dnnl_memory_create(&fc_diff_bias_memory, fc_diff_bias_md, engine,
                DNNL_MEMORY_ALLOCATE));
        CHECK(dnnl_memory_set_data_handle(
                fc_diff_bias_memory, fc_diff_bias_buffer));
        
        // finally created backward convolution weights primitive
        dnnl_primitive_t fc_bwd_weights;
        CHECK(dnnl_primitive_create(&fc_bwd_weights, fc_bwd_weights_pd));
        net_bwd[*n_bwd] = fc_bwd_weights;
        ...
        set_arg(&net_bwd_args[*n_bwd].args[2], DNNL_ARG_DIFF_WEIGHTS,
                fc_diff_weights_memory);
        set_arg(&net_bwd_args[*n_bwd].args[3], DNNL_ARG_DIFF_BIAS,
                fc_diff_bias_memory);
        *n_bwd++;
       if (fc_reorder_diff_weights) *n_bwd += 1;
        prv_layer return_val = { .dst_md = fc_diff_dst_md, .dst_memory = fc_diff_dst_memory, .src_md = layer_i.clayer.src_md, .src_memory = layer_i.clayer.src_memory };

        return return_val;
}

prv_layer create_bwd_avgpool_layer(dnnl_engine_t engine, uint32_t *n_bwd, dnnl_primitive_t *net_bwd, args_t *net_bwd_args, layer_info layer_i){
        ...
        const_dnnl_memory_desc_t pool_diff_src_md = layer_i.clayer.src_md;
        const_dnnl_memory_desc_t pool_diff_dst_md = layer_i.clayer.dst_md;
        ...
        if(layer_i.p > 0){
                CHECK(dnnl_pooling_backward_primitive_desc_create(&pool_bwd_pd, engine,
                        dnnl_pooling_avg_include_padding, pool_diff_src_md, pool_diff_dst_md, pool_strides,
                        pool_kernel, pool_dilation, pool_padding, pool_padding, layer_i.clayer.fwd_hint,
                        NULL));
        }
        else{
                CHECK(dnnl_pooling_backward_primitive_desc_create(&pool_bwd_pd, engine,
                        dnnl_pooling_avg_exclude_padding, pool_diff_src_md, pool_diff_dst_md, pool_strides,
                        pool_kernel, pool_dilation, pool_padding, pool_padding, layer_i.clayer.fwd_hint,
                        NULL));
        }
        ...
        set_arg(&net_bwd_args[*n_bwd].args[0], DNNL_ARG_DIFF_DST,
                layer_i.player.src_memory);
        set_arg(&net_bwd_args[*n_bwd].args[1], DNNL_ARG_WORKSPACE, layer_i.clayer.ws_memory);
        set_arg(&net_bwd_args[*n_bwd].args[2], DNNL_ARG_DIFF_SRC,
                pool_diff_src_memory);
        *n_bwd++;

        prv_layer return_val = { .dst_md = pool_diff_dst_md, .dst_memory = layer_i.player.src_memory, .src_md = pool_diff_src_md, .src_memory = pool_diff_src_memory };
        return return_val;
}

prv_layer create_bwd_maxpool_layer(dnnl_engine_t engine, uint32_t *n_bwd, dnnl_primitive_t *net_bwd, args_t *net_bwd_args, layer_info layer_i){
        dnnl_dims_t pool_kernel = {layer_i.k, layer_i.k};
        dnnl_dims_t pool_strides = {layer_i.s, layer_i.s};
        dnnl_dims_t pool_padding = {layer_i.p, layer_i.p};
        dnnl_dims_t pool_dilation = {layer_i.d, layer_i.d};
...
prv_layer create_bwd_relu_layer(dnnl_engine_t engine, uint32_t *n_bwd, dnnl_primitive_t *net_bwd, args_t *net_bwd_args, layer_info layer_i){
        float negative_slope = 0.0f;
        
        // Backward relu
        const_dnnl_memory_desc_t relu_diff_src_md = layer_i.player.src_md;
        const_dnnl_memory_desc_t relu_diff_dst_md = layer_i.player.src_md;
        ...
        CHECK(dnnl_eltwise_backward_primitive_desc_create(&relu_bwd_pd, engine,
                dnnl_eltwise_relu, relu_diff_src_md, relu_diff_dst_md, layer_i.clayer.src_md,
                negative_slope, 0, layer_i.clayer.fwd_hint, NULL));
        ...
        *n_bwd++;
        ...
        prv_layer return_val = { .dst_md = relu_diff_dst_md, .dst_memory = layer_i.player.src_memory, .src_md = relu_diff_src_md, .src_memory = relu_diff_src_memory };
        return return_val;
}

prv_layer create_bwd_conv_layer(dnnl_engine_t engine, uint32_t *n_bwd, dnnl_primitive_t *net_bwd, args_t *net_bwd_args, layer_info layer_i){
        dnnl_dims_t conv_user_src_sizes = { BATCH, layer_i.in_c, layer_i.in_h, layer_i.in_w };
        dnnl_dims_t conv_user_weights_sizes = {layer_i.out_c, layer_i.in_c, layer_i.k, layer_i.k};
        dnnl_dims_t conv_bias_sizes = {layer_i.out_c};
        dnnl_dims_t conv_user_dst_sizes = {BATCH, layer_i.out_c, layer_i.in_h, layer_i.in_w};
        dnnl_dims_t conv_strides = {layer_i.s, layer_i.s};
        dnnl_dims_t conv_dilation = {layer_i.d, layer_i.d};
        dnnl_dims_t conv_padding = {layer_i.p, layer_i.p};
        ...
        // initialize memory for diff weights in user format
        dnnl_memory_t conv_user_diff_weights_memory; // <- 이거가 conv_user_wieghts_memory여야 함
        ...
        dnnl_primitive_desc_t conv_bwd_weights_pd;
        {
                ...
                CHECK(dnnl_convolution_backward_weights_primitive_desc_create(
                        ...
                        conv_padding, layer_i.clayer.fwd_hint, NULL));
        ...
        CHECK(prepare_reorder(&layer_i.clayer.src_memory, conv_diff_src_md, engine, 1,
        ...
        CHECK(prepare_reorder(&layer_i.player.src_memory, conv_diff_dst_md, engine, 1,
        ...
        const_dnnl_memory_desc_t conv_diff_weights_md
                = dnnl_primitive_desc_query_md(conv_bwd_weights_pd, dnnl_query_diff_weights_md, 0);
        if (layer_i.layer_order==First){
        ...
        *n_bwd++;
        ...
        prv_layer return_val = { .dst_md = conv_diff_dst_md, .dst_memory = conv_diff_dst_memory, .src_md = conv_diff_src_md, .src_memory = conv_bwd_weights_src_memory };
        return return_val;
}
...
void execute_model(FILE* model_file) {
    ...
    dnnl_primitive_t net_fwd[n_mod], net_bwd[n_mod];
    args_t net_fwd_args[n_mod], net_bwd_args[n_mod];
    ...
    // info for each layer
    int cur_cmd;
    std::stack<std::pair<int, layer_info>> bwd_cmds_stack;
    ...
    for(int mod_i=0; mod_i<n_mod; mod_i++){
        ...
        switch(cur_cmd){
            ...
            case MaxPool2d:
                layer_args.d -= 1;
                // out_h, out_w 차원 계산
                layer_args.out_h = (int)floor((layer_args.in_h-(layer_args.d+1)*(layer_args.k-1)+2*layer_args.p)/layer_args.s)+1;
                layer_args.out_w = (int)floor((layer_args.in_w-(layer_args.d+1)*(layer_args.k-1)+2*layer_args.p)/layer_args.s)+1;
                layer_args.out_c = layer_args.in_c;
        ...
        // backward stack에 넣기
        bwd_cmds_stack.push({cur_cmd, layer_args});
    }
    dnnl_memory_t net_user_dst_memory = tmp_layer_md.dst_memory;
    // backward stream
    // user diff_data
    float *net_diff_dst = (float*)malloc(product(net_dst_sizes, 4) * sizeof(float));
    init_net_data(net_diff_dst, 4, net_dst_sizes);
    
    while(!bwd_cmds_stack.empty()){
        std::pair<int, layer_info> cur_bwd_cmd = bwd_cmds_stack.top();
        bwd_cmds_stack.pop();
        if(n_bwd!=0){ // 일단 여기서 player은 이 while문 내에서 생성된거고, clayer는 forward 시에 생성된 해당 레이어에 대한 정보임
                cur_bwd_cmd.second.player = tmp_layer_md;
        }
        switch(cur_bwd_cmd.first){
            case Conv2d:
                tmp_layer_md = create_bwd_conv_layer(engine, &n_bwd, net_bwd, net_bwd_args, cur_bwd_cmd.second);
                break;
            case ReLU:
                tmp_layer_md = create_bwd_relu_layer(engine, &n_bwd, net_bwd, net_bwd_args, cur_bwd_cmd.second);
                break;
            case MaxPool2d:
                tmp_layer_md = create_bwd_maxpool_layer(engine, &n_bwd, net_bwd, net_bwd_args, cur_bwd_cmd.second);
                break;
            case AdaptiveAvgPool2d:
                tmp_layer_md = create_bwd_avgpool_layer(engine, &n_bwd, net_bwd, net_bwd_args, cur_bwd_cmd.second);
                break;
            case Linear:
                tmp_layer_md = create_bwd_fc_layer(engine, &n_bwd, net_bwd, net_bwd_args, cur_bwd_cmd.second, net_diff_dst);
                break;
            default:
                break; 
        }
    ...
    for (int ni = 0; ni < n_iter; ni++) {
        ...
        CHECK(dnnl_memory_get_data_handle(net_user_dst_memory, &net_output));
...
int main(int argc, char **argv) {
    FILE* model_file = fopen("/home/ydy/oneDNN/examples/check_vgg_model2.txt", "r");
    ariel_enable();
