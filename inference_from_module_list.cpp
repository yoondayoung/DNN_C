...
prv_layer create_bn_layer(dnnl_engine_t engine, uint32_t *n_fwd, dnnl_primitive_t *net_fwd, args_t *net_fwd_args,  layer_info layer_i){
        ...
        // allocate src, weight, bias, dst
        float *scales = (float*)mlm_malloc(product(scaleshift_dims, 1) * sizeof(float), 1);
        float *shifts = (float*)mlm_malloc(product(scaleshift_dims, 1)*sizeof(float), 1);
        // initialiaze weight, bias
...
void execute_model(FILE* model_file) {
    ...
    float *net_src
            = (float *)mlm_malloc(product(net_src_sizes, ndims) * sizeof(float), 1);
    float *net_dst
            = (float *)mlm_malloc(product(net_dst_sizes, 2) * sizeof(float), 1);
